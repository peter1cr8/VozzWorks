...
import com.vozzware.db.util.VwColAlias;
import com.vozzware.db.util.VwColPropertyGlob;
...
public class VwSchemaObjectMapper
{
    ...
    try
    {
     ...
     // *** Check for overrides
      ...
      if ( dbCommon != null )
      {
        ...
        if ( m_strTableName != null && (strClassName == null || (dbCommon.getGenDvoFromTable() != null && dbCommon.getGenDvoFromTable().equals( "true" )) ))
        {
      ...
      if ( fGenCode || ( fileJava != null && !fileJava.exists() ) )
      {
        ...
        if ( dvoGen != null )
        {
          ...
          {
            List<VwColAlias>colAliases = null;

            if ( dbCommon instanceof VwTableSpec )
            {
              colAliases = ((VwTableSpec)dbCommon).getJsonAlias();
            }
            buildJavaClassFromTableCols( dvoGen, listTableColumns, tr, listPrimeKeySuppliers, vwExtendsHierarchyDescriptor, colAliases );
          }
          else
  ...
  private List<VwColInfo> getTableColumns( VwDbObjCommon dbCommon ) throws Exception
  {
    ...
    if ( dbCommon instanceof VwDbQuery )
    {
      ...
      String strSQL = ((VwDbQuery)dbCommon).getSql();
      ...
      return getColListFromQuery( strSQL );
    ...
    return getColListFromResultSet( (VwTableSpec)dbCommon );

  } // end getTableColumns()


  private List<VwColInfo> getColListFromResultSet( VwTableSpec tableSpec ) throws Exception
  {
    Map<String,String> mapUserDefPrimeKeyCols = null;
    ...
    String strPrimeKeyCols = tableSpec.getPrimaryKeyCols();
    ...
    DatabaseMetaData dbm =  m_sqlMgr.getDatabase().getMetaData();
    ResultSet rs = dbm.getColumns( null, null, m_strTableName, null  );
    ...
    List<VwColInfo> listColumns = new ArrayList<>();;
    // build column list from result set meta data
    while( rs.next() )
    {
      //String strColName = md.getColumnLabel( x + 1 );
      String strColName = rs.getString( "COLUMN_NAME" );
      ...
      ci.setSQLType( rs.getInt( "DATA_TYPE") );
      ci.setNbrDecimalDigits( rs.getInt( "DECIMAL_DIGITS" ) );
      ci.setColSize( rs.getInt( "COLUMN_SIZE") );

      String strDefValue = rs.getString( "COLUMN_DEF" );

      if ( strDefValue != null )
      {
        int npos = strDefValue.indexOf( "::" );
        if ( npos >= 0 )
        {
          strDefValue = strDefValue.substring( 0,npos );
        }

        strDefValue = VwExString.strip( strDefValue, "'");

      }

      ci.setDefValue( strDefValue );
  ...
  } // end getColListFromResultSet()

  /**
   * Gets the column list from a SQL Query
   *
   * @param strSQL
   * @return
   * @throws Exception
   */
  private List<VwColInfo> getColListFromQuery( String strSQL ) throws Exception
  {
    try
    {
  ...
  private void buildJavaClassFromTableCols( VwDVOGen dvoGen, List<VwColInfo> listColumns, VwTableRelationship tr,
                                            List<VwKeyDescriptor> listPrimeKeySuppliers,
                                            VwExtendsHierarchyDescriptor extendsHierarchyDescriptor,
                                            List<VwColAlias>listColAliases) throws Exception
  {
    ...
    for ( VwColInfo ci : listColumns )
    {
      ...
      String strColAlias = getColAlias( strPropName, listColAliases );

      String strJsonAnnotation = null;

      if ( strColAlias != null )
      {
        strJsonAnnotation = genJsonPropAliasAnnotation( strColAlias );
      }
      ...
      propDef.setInitialValue( ci.getDefValue() );
      propDef.setAnnotation( strJsonAnnotation );
  ...
  private String getColAlias( String strPropsName, List<VwColAlias> listColAliases )
  {
    if ( listColAliases != null )
    {

      for ( VwColAlias colAlias : listColAliases )
      {
        if ( colAlias.getName().equals( strPropsName ) )
        {
          return colAlias.getValue();
        }
      }
    } // end if

    if ( m_spec.getJsonPropertyGlob() == null )
    {
      return null;
    }

    String strPropGlob = null;

    for ( VwColPropertyGlob propGlob : m_spec.getJsonPropertyGlob() )
    {
      switch( propGlob.getFrom().charAt( 0 ) )
      {
        case '$': // ends with

          strPropGlob = doEndsWith( strPropsName, propGlob );
          break;

        case '^': // ends with

          strPropGlob = doStartsWith( strPropsName, propGlob );
          break;

        case '~': // ends with

          strPropGlob = doContains( strPropsName, propGlob );
          break;
  ...
  /**
   * Replace the contains string with the to property
   * @param strPropsName The prop name to test
   * @param propGlob The propglob
   * @return
   */
  private String doContains( String strPropsName, VwColPropertyGlob propGlob )
  {
    String strSearchVal = propGlob.getFrom().substring( 1 );

    int nPos = strPropsName.indexOf( strSearchVal );

    if ( nPos > 0 )
    {
      String strTo = propGlob.getTo();
      String strReplaceVal = strPropsName.substring( 0, nPos );;

      if ( strTo.length() == 0 )
      {
         strReplaceVal += strPropsName.substring( nPos + strSearchVal.length() );
      }
      else
      {
        strReplaceVal += strTo + strPropsName.substring( nPos + strSearchVal.length() );
      }
  ...
  /**
   * Replace the startsWith string with the to property
   * @param strPropsName The prop name to test
   * @param propGlob The propglob
   * @return
   */
  private String doStartsWith( String strPropsName, VwColPropertyGlob propGlob )
  {
    String strSearchVal = propGlob.getFrom().substring( 1 );

    if ( strPropsName.startsWith( strSearchVal ) )
    {
      String strTo = propGlob.getTo();
      return strPropsName.replace( strSearchVal, strTo );
  ...
  /**
   * Replace the endsWith string with the to property
   * @param strPropsName The prop name to test
   * @param propGlob The propglob
   * @return
   */
  private String doEndsWith( String strPropsName, VwColPropertyGlob propGlob )
  {
    String strSearchVal = propGlob.getFrom().substring( 1 );

    if ( strPropsName.endsWith( strSearchVal ) )
    {
      String strTo = propGlob.getTo();
      return strPropsName.replace( strSearchVal, strTo );
  ...
  private String genJsonPropAliasAnnotation( String strPropAliasName )
  {
    return "@VwJsonProperty( prop = \""  + strPropAliasName + "\" )";

  } // end genPropAliasAnnotation()
