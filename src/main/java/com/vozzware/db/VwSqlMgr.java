...
public class VwSqlMgr
{
  ...
  public void deleteAll( Object objToDelete, boolean fDoTimeStampCheck ) throws VwTimestampOutOfSyncException, Exception
  {
    ...
    for ( PropertyDescriptor pd : listProps )
    {
     ...
     {
        Object objReturn = mthdRead.invoke( objToDelete );
        if ( objReturn != null )
        {
          deleteAll( objReturn );
        }
  ...
  public boolean exists( Object objToTest ) throws Exception
  {
    ...
    // we have to sve dirty state because the exec method clears the dirty fileds
    boolean bIsDirtyObject = true;
    if ( objToTest instanceof VwDVOBase )
    {
      bIsDirtyObject = ((VwDVOBase)objToTest).isDirty( );
    }

    exec( strSql, objToTest );

    // Restore the dirty fields state
    if ( objToTest instanceof VwDVOBase )
    {
      ((VwDVOBase)objToTest).setDirty( bIsDirtyObject );
    }
   ...
   * 
   * @param objToSave
   * @param bRemoveDeletedObjects
   * @param bDoTimestampCheck performs a timestamp comparison test. See the description in the syncDelete method
   * @param strMappingId
   * @return
   * @throws Exception
   */
  private boolean saveObject( Object objToSave, boolean bRemoveDeletedObjects, boolean bDoTimestampCheck, String strMappingId  )  throws VwTimestampOutOfSyncException, Exception
  {

    Class clsObjToSave = findClassSqlMapping( objToSave.getClass() );

    boolean bDoExistsTest = false;
    ...
    if ( vwExtendsDesc != null )
    {
      ...
      saveObject( objToSave, bRemoveDeletedObjects, bDoTimestampCheck, vwExtendsDesc.getSuperClass() );
    ...
    if ( listForeignKeys != null )
    {
      for ( Iterator<VwKeyDescriptor> iKeys = listForeignKeys.iterator(); iKeys.hasNext(); )
      {
        ...
        boolean fDeleted = saveObject( objForeign, bRemoveDeletedObjects, bDoTimestampCheck, strMappingId );

        if ( fDeleted )
        {
          if ( bRemoveDeletedObjects )
          {
    ...
    {
      doUpdate( objToSave, strMappingId, bDoTimestampCheck );
    }
      ...
      for ( Iterator iKeySup = listPrimeKeySuppliers.iterator(); iKeySup.hasNext(); )
      {
        ...
        if ( objVal instanceof Collection )
        {
          ...
          for ( Iterator<?> iObj = list.iterator(); iObj.hasNext(); )
          {
            ...
            boolean fDeleted = saveObject( objToSupply, bRemoveDeletedObjects, bDoTimestampCheck, null );
            if ( fDeleted )
            {
              if ( bRemoveDeletedObjects )
              {
        ...
        else
        {
          ...
          saveObject( objVal, bRemoveDeletedObjects, bDoTimestampCheck, strMappingId );
  ...
  public void setPrimaryKeyFromOracleSeq( Object objToSave, VwPrimaryKeyGeneration primeKey ) throws Exception
  {
    ...
    try
    {
      ...
      VwBeanUtils.setValue( objToSave, strKeyProp, Long.valueOf( lSeq ) );
    ...
    for ( int x = 0; x < m_sqlData.m_listResults.size(); x++ )
    {
        ...
        switch( m_sqlData.m_asResultTypes[ x ] )
        {
          ...
          case Types.TIME:
               ...
               {
                 Method mthdTimeStamp = objDate.getClass().getMethod( "timestampValue" );
                 
                 Timestamp tm = (Timestamp)mthdTimeStamp.invoke( objDate );
                 dt = new VwDate( tm );
