 ...
 * <br/>vw.log4j.db.category A user assigned catagory (may be null) for searching
...
public class VwLogger
{
  ...
  private static final Map<String, Level> s_mapLevels = new HashMap<>();

  private static final Map<String,VwLogger> s_mapInstances = Collections.synchronizedMap( new HashMap<>() );
  ...
  public VwLogger( String strPropFileName ) throws Exception
  {
    ...
    if (  m_strInstanceName.endsWith( ".properties" ))
    {
  ...
  /**
   * Gets the singleton instance with the default configuration that only has a console appender and sets the out level
   * to debug (the lowest)
   *
  ...
  /**
   * Gets the instance based on the instance name passed. If a logger exists with this name then that instance is returned.
  ...
  public synchronized static VwLogger getInstance( String strInstanceName ) throws Exception
  {
    ...
    {
      logger = s_mapInstances.get( strInstanceName );
    }
    else
    {
      logger = s_mapInstances.get( "DEFAULT" );
    }
   ...
   *
   * @param clsProducer The name of the class that produced the message
  ...
  public boolean removeAppender( String strAppenderName )
  {
  ...
  private void configureFromProps() throws Exception
  {
    ...
    m_loggerConfig = new LoggerConfig( m_strInstanceName, Level.INFO, false );
    ...
    m_instanceLayout =  PatternLayout.newBuilder().withPattern(strPatternLayout)
      .withConfiguration(m_ctx.getConfiguration()).build();
  ...
  protected void createLogFileAppender()  throws Exception
  {
    ...
    Appender logFileAppender = FileAppender.newBuilder().withFileName( strLogFile ).withAppend( true ).withLocking( true )
      .setImmediateFlush( true ).setBufferedIo( true ).setBufferSize( 8192 ).setName( "LOG_FILE_APPENDER" )
      .setBufferedIo( true ).setLayout( m_instanceLayout ).setConfiguration( m_config ).build();
  ...
  protected void createRollingLogFileAppender()  throws Exception
  {
    ...
    DefaultRolloverStrategy strat = DefaultRolloverStrategy.newBuilder().withConfig( m_config ).withMin( "1" ).withMax( strMaxRollOvers ).withStopCustomActionsOnError( false ).build();
    ...
    Appender logFileAppender = RollingFileAppender.newBuilder().withFileName( strRollingLogFile ).withAppend( true ).withLocking( true )
      .withPolicy( policy ).withStrategy( strat ).setImmediateFlush( true ).setBufferedIo( true ).setBufferSize( 8192 ).setName( "VW_ROLLING_FILE_APPENDER" )
      .setBufferedIo( true ).setLayout( m_instanceLayout ).setConfiguration( m_config ).build();
