...
public class VwDocFinder
{
   ...
   */
  public static URL findURL( String strPropName, String strDocName ) throws Exception
  { return findURLClassPath( strPropName, strDocName ); }
  
  /**
   * Finds a document using the default VWDOCS path list property and returns a URL to the document
   * 
   * @param strDocName The name of the document to find
   * @return A URL to the document found
   * @throws Exception if the property is not found
   * @throws FileNotFoundException if the document cannot be found
   */
  public static URL findURL( String strDocName ) throws Exception
  ...
  {
    if ( strDocName.startsWith( "file:") )
    {
      return new URL( strDocName );
    }
    else
    if ( strDocName.startsWith( "/") )
    {
      String strPath = System.getenv( "project_loc" );
