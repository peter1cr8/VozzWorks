...
public class VwXmlToBean extends DefaultHandler implements XmlDeSerializer
{
  ...
  private Map<Class,Map<String,TagMethodInfo>>m_mapAnnoMethodsByClass = new HashMap<>();
   ...
   * @param schema An XML schema/DTD to use for deSerialization help (May be null )
   *
   * @return an object of the top level class type containing the deSerialized XML document
   */
   ...
   *
   * @param strXmlTagName The xml element name
   * @param clsAlias
   */
   ...
   * @param clsAlias The class to handle this element
   */
   ...
   * @param iCloseElementListener The implementing listener class
   *
   * @throws Exception
   */
   ...
   * @param strElementName The name of the XML element to listen for
   * @param strURI The associated URI for this element (may be null if N/A)
   * @param iOpenElementListener The implementing listener class
   *
  ...
  */
  ...
  protected void introspect( Class classToIntrospect ) throws Exception
  {
    ...
    Map<String,TagMethodInfo>mapAnnoMethods = m_mapAnnoMethodsByClass.get( classToIntrospect );

    if ( mapAnnoMethods == null )
    {
      mapAnnoMethods = new HashMap<>();
      m_mapAnnoMethodsByClass.put( classToIntrospect, mapAnnoMethods );
    }
    ...
    for ( int x = 0; x < aMethods.length; x++ )
    {
      ...
      if ( strMethodName.startsWith( "add" ) || strMethodName.startsWith( "set" ) )
      {
        ...
        {
          TagMethodInfo tmi = new TagMethodInfo( strAlias, method, clsParam, SIMPLE, fHasTagHandler );
          mapClassMethods.put( strMethodName.toLowerCase(), tmi  );

          VwJsonProperty jsonProp = method.getAnnotation( VwJsonProperty.class );
          if ( jsonProp != null )
          {
            mapAnnoMethods.put( jsonProp.prop(), tmi );
          }
        }
        else
                                        ...
                                        Class clsCollectionContains, boolean fObjAlias ) throws Exception
  {
    ...
    if ( isGenericType )
    {
      String strParamType = VwBeanUtils.getGenericParameterType( method, 0  );
      String[] astrParamTypes = strParamType.split( ",");

      // if array type genth = 2 then this is a map so astrParamTypes[1] is the object the map contains
      if ( astrParamTypes.length == 2  )
      {
        clsCollectionContains = Class.forName( astrParamTypes[1].trim() );
      }
      else
      {
        clsCollectionContains = Class.forName( strParamType );
      }
    }
    ...
    if ( !fSimpleType && !VwBeanUtils.isCollectionType( clsCollectionContains ) )
    {
  ...
  public void startElement( String strUri, String strName, String strQName, Attributes attrList ) throws SAXException
  {
    ...
    if ( tmi == null && m_objCollection != null)
    {
      ...
      if ( m_tagInfo != null )
      {
        ...
        if ( m_tagInfo.m_nType == OBJECT && m_tagInfo.m_strParamClassName.equalsIgnoreCase( strName ) )
        {
          try
          {
            setupObject( listAttr, listNameSpaces );
  ...
  public void endElement( String strUri, String strName, String strQName ) throws SAXException
  {
    ...
    try
    {
      ...
      // get map for anny json annotation property overrides
      Map<String,TagMethodInfo>mapAnnoProps = m_mapAnnoMethodsByClass.get( m_curObj.getClass() );
      ...
      else
      {
        ...
        if ( tmi == null )
        {
          if ( mapAnnoProps != null )
          {
            tmi = mapAnnoProps.get( strName.toLowerCase() );
          }
        }
        ...
        if ( tmi != null )
        {
          ...
          else
          {
            ...
            if ( tmi.m_nType == SIMPLE )
            {
              try
              {
                invokeSetterMethod( tmi, getTagData( m_strCurTagName ) );
              }
              catch( Exception e )
              {
                throw new SAXException( e );
   ...
   *
   * @param  objKey The name of the Map value tag
   */
  ...
  private void addToSimpleCollection( TagMethodInfo tmi ) throws Exception
  {
    ...
    if ( tmi.m_clsParamType == Boolean.class || tmi.m_clsParamType == Boolean.TYPE )
    {
      ...
      obj = Boolean.valueOf( fState );
    }
    else
    if ( tmi.m_clsParamType == Byte.class || tmi.m_clsParamType == Byte.TYPE )
      obj = (byte) strTagData.charAt( 0 );
    else
    if ( tmi.m_clsParamType == Character.class || tmi.m_clsParamType == Character.TYPE )
      obj = (char)strTagData.charAt( 0 );
    else
    if ( tmi.m_clsParamType == Short.class || tmi.m_clsParamType == Short.TYPE  )
      obj = Short.valueOf( strTagData );
    else
    if ( tmi.m_clsParamType == Integer.class || tmi.m_clsParamType == Integer.TYPE )
      obj = Integer.valueOf( strTagData );
    else
    if ( tmi.m_clsParamType == Long.class || tmi.m_clsParamType == Long.TYPE )
      obj = Long.valueOf( strTagData );
    else
    if ( tmi.m_clsParamType == Float.class || tmi.m_clsParamType == Float.TYPE )
      obj = Float.valueOf( strTagData );
    else
    if ( tmi.m_clsParamType == Double.class  || tmi.m_clsParamType == Double.TYPE )
      obj = Double.valueOf( strTagData );
    else
    if ( tmi.m_clsParamType == java.math.BigInteger.class )
