...
public class VwClassGen
{
  ...
  class DataMbr
  {
    ...
    String m_strAnnotation; // property annotation if specified
    ...
    DataMbr( String strName, DataType eDataType, String strUserDefType, int[] anDimList, String strComment,
             String strInitValue, int nDataFlags, String strAnnotation )
    {
      m_strName = strName;
  ...
  class VwMethod
  {
    ...
    VwMethod( String strName, String strMangledName, DataType eRetDataType,
              ...
              int nRetDataFlags, int nRetNbrDim, MethodParams[] aParams, String strExceptionDoc, String strAnnotation )
    {
  ...
  private boolean genDataMembers( ScopeDesc curScopeTbl )
  {
    ...
    while ( itr.hasNext() )
    {
      ...
      if ( dataMbr.m_strInitValue != null && !dataMbr.m_strInitValue.startsWith( "nextval(") )
      {
        if (dataMbr.m_strInitValue.equals( "now()") )
        {
          dataMbr.m_strInitValue = "new Date()";
        }
        strName += " = ";
        ...
        else
        {
          ...
          if ( dataMbr.m_eDataType == DataType.LONG_OBJ )
          {
            strName += "L";
          }
  ...
  {
    VwMethod method; // Buffer to hold one VwMethod node

    String strIndent = m_strIndent; // level of indentation

    // *** Create iterator to loop thru linked List

    Iterator iMethods = curScopeTbl.m_listMethodsList.iterator();

    try
    {
      while ( iMethods.hasNext() )
      {
        method = (VwMethod) iMethods.next();
        ...
        sbMethod.append( strRetType).append( " " ).append( buildMethodSigniture( method.m_strName, method.m_strAnnotation, method.m_aParams ) );

        if ( method.m_strExceptions != null )
        {
          sbMethod.append(" throws " ).append( method.m_strExceptions );
        }

        m_out.write( sbMethod.toString(), 0, sbMethod.length() ); // Write method declaration header

        if ( ( m_nObjectType & INTERFACE ) == INTERFACE )
        {
          sbMethod = new StringBuffer(";" );  // method ends here if this is an interace
        }
        else
        {
          ...
          {
            sbMethod = new StringBuffer("\n");
            sbMethod.append( strIndent ).append( "{\n" );
          }

          // *** Add in method code ***

          sbMethod.append( method.m_strCode );

          // *** Add in closing brace

          sbMethod.append( "\n" ).append( strIndent ).append( "} // End of " ).append( method.m_strName ).append( "()\n\n" );
   ...
   * @param strAnnotation The methgod annotation code (if specified )
   * @param aParams - The array of method parameters or null if no parameters
   * @return The complete method signiture in a String
   */
  private String buildMethodSigniture( String strName, String strAnnotation, MethodParams[] aParams )
  {
    StringBuffer sbMethodSigFun = new StringBuffer();
    ...
    if ( aParams == null )
    {
      ...
      {
        sbMethodSigFun.append( strAnnotation + " " );
      }

      sbMethodSigFun.append( strName );
      sbMethodSigFun.append("()" );

      return sbMethodSigFun.toString();

    } // end if

    // *** build param List with each parameter entry found

    StringBuffer sbParamList = new StringBuffer();
    ...
    sbMethodSigFun.append( strName ); // Copy base member function name
    sbMethodSigFun.append( "( " ); // Add left open paren
    ...
    for ( int ndx = 0; ndx < aParams.length; ndx++ )
    {
      ...
      sbParamList.append(buildDataType( -1, curParam.m_nFlags, curParam.m_eDataType, curParam.m_nNbrDim,
      ...
      // Add hungarian prefix if specified for function prototype name
      if ( m_codeOpt.m_fUseHungarian && ( curParam.m_nFlags & ISLIT ) != ISLIT )
       {
        strHung += getHungPrefix( curParam.m_nFlags, curParam.m_eDataType );
      }

      sbParamList.append( strHung).append( curParam.m_strName ); // Add param naame

    } // end for()

    sbMethodSigFun.append( sbParamList).append( " )" );
  ...
  private boolean genSetMbr( DataMbr dataMbr )
  {
    ...
    if ( dataMbr.m_strAnnotation != null )
    {
      sbSet.append( "\n" ).append( strIndent ).append( dataMbr.m_strAnnotation );

    }

    sbSet.append( "\n" ).append( strIndent );
    sbSet.append( buildDataType( PUBLIC, 0, DataType.VOID, 0, null, false ) );
    ...
    {
      strSmartSetCode = "testDirty( \"" + dataMbr.m_strName + "\"";
    }

    sbSet.append( " set" ).append( strPropName ).append( "( " ); // Add base data name to set
    ...
    sbSet.append( strType ).append( " " );

    if ( dataMbr.m_eDataType == DataType.USERDEF && strHung.length() == 0 )
    {
      sbSet.append( strName );
    }
    else
    {
      if ( strTypeName != null )
      {
        sbSet.append( strTypeName );
      }
      else
      {
        sbSet.append( strName );
    ...
    if ( strSmartSetCode != null )
    {
      ...
      sbSet.append( strSmartSetCode );
    }

    sbSet.append( strMbrPre ).append( strName ).append( " = " );

    sbSet.append( strParamName ).append( ";" );
    ...
    VwMethod method = new VwMethod( "set" + dataMbr.m_strName, null, DataType.VOID, "Sets the " + dataMbr.m_strName
        + " property", null, null, null, null, 0, 0, 0, aParams, null, null );

    try
    {
      writeDoc( method );
      m_out.write( sbSet.toString(), 0, sbSet.length() );
    }
    catch ( IOException iox )
    {
  ...
  private boolean genGetMbr( DataMbr dataMbr )
  {
    ...
    sbGet.append( "\n" ).append( strIndent );
    sbGet.append( buildDataType( PUBLIC, nFlags, dataMbr.m_eDataType, 0, dataMbr.m_strUserDefType, false ) );
    ...
    sbGet.append(" get" ).append( strPropName ).append( "(" ); // Add base data name to set
    ...
    sbGet.append( "\n" ).append( strIndent ).append( "{ return " ).append( strMbrPre ).append( strName );
    sbGet.append( "; }" );

    // *** Create a method instance so we can also write javadoc for our
    // generated setXXXX method

    VwMethod method = new VwMethod( "get" + strPropName, null, dataMbr.m_eDataType, "Gets " + dataMbr.m_strName
    ...
    try
    {
      ...
      m_out.write( sbGet.toString(), 0, sbGet.length() );
    }
    catch ( IOException iox )
    {
      m_strErrDesc = iox.toString();
   ...
   */
  private void writeDoc( VwMethod method ) throws IOException
  {
  ...
  private void genConstructors( int nScope )
  {
    ...
    try
    {
      while ( itr.hasNext() )
      {
        ...
        VwMethod method = new VwMethod( m_strClassName, null, null, "Constructs the " + m_strClassName + " instance",
                                        null, null, null, null, 0, 0, 0, construct.m_aParams,
                                        null, null );

        writeDoc( method );

        str += buildMethodSigniture( m_strClassName, null, construct.m_aParams );
        ...
        {
          str += m_strIndent + m_strIndent + buildMethodSigniture( "super", null, construct.m_aSuperParams ) + ";\n";
        }
  ...
  public boolean addDataMbr( String strDataName, DataType eDataType,
                             ...
                             String strInitValue, int[] anDimList, String strUserDefType, String strAnnotation )
  {

    DataMbr mbr = new DataMbr( strDataName, eDataType, strUserDefType, anDimList,
                               strComment, strInitValue, nDataFlags, strAnnotation );   // Allocate
    // DataMbr
    // instance
  ...
  {
    VwMethod method = new VwMethod( strMethodName, getMangledName( strMethodName, aMethodParams ), eRetDataType,
                                    strMethodDoc, strReturnDoc, strCode, strUserDefRetType, strExceptions, nFlags, nRetDataFlags, nRetNbrDim,
                                    aMethodParams, strExceptionDoc, null );
